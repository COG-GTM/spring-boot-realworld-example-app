...
public class UserWithTokenTest {
  ...
  @Test
  public void should_not_serialize_the_user_id() throws JsonProcessingException {
    UserData userData = new UserData("user-id", "jane@example.com", "jane", "bio", "image");

    String json = new ObjectMapper().writeValueAsString(new UserWithToken(userData, "token"));

    assertThat(json).doesNotContain("user-id");
    assertThat(json).contains("\"email\":\"jane@example.com\"", "\"token\":\"token\"");
  }
