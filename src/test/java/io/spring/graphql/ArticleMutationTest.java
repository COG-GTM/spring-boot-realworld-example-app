...
public class ArticleMutationTest {
  ...
  @Test
  public void should_throw_exception_when_deleting_article_without_authorization() {
    String slug = "test-title";
    User differentUser = new User("other@test.com", "otheruser", "pass", "bio", "img");
    
    when(articleRepository.findBySlug(slug)).thenReturn(Optional.of(testArticle));

    SecurityContextHolder.getContext().setAuthentication(
        new UsernamePasswordAuthenticationToken(differentUser, null, Collections.emptyList()));

    Assertions.assertThrows(NoAuthorizationException.class, () -> {
