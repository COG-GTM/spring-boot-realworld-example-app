...
public class DefaultJwtService implements JwtService {
  ...
  public DefaultJwtService(
    ...
    this.signatureAlgorithm = strongestAlgorithmFor(secret.getBytes().length * 8);
    this.signingKey = new SecretKeySpec(secret.getBytes(), jcaName(signatureAlgorithm));
  }

  private static MacAlgorithm strongestAlgorithmFor(int keyBits) {
    if (keyBits >= 512) {
  ...
  private static String jcaName(MacAlgorithm algorithm) {
    return "HmacSHA" + algorithm.getId().substring(2);
  }
