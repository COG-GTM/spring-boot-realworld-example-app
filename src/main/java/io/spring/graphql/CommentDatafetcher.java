...
public class CommentDatafetcher {
  ...
  private Comment buildCommentResult(CommentData comment) {
    return Comment.newBuilder()
        ...
        .updatedAt(DateTimeFormatter.ISO_DATE_TIME.withZone(ZoneOffset.UTC).format(comment.getCreatedAt()))
        .createdAt(DateTimeFormatter.ISO_DATE_TIME.withZone(ZoneOffset.UTC).format(comment.getCreatedAt()))
        .build();
  }
}
